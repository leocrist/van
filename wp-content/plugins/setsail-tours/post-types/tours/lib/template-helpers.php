	...
	function setsail_tours_get_tour_info_table_data($tour_id = null) {
		...
		if(!empty($destination_option)) {
				...
				$destination_item = array(
					'text'  => esc_html__('Reiseform', 'setsail-tours'),
					'value' => $destination_option
				);
		...
		if(!empty($departure_option)) {
			$departure_item = array(
				'text'  => esc_html__('Erlebnisse', 'setsail-tours'),
		...
		if(!empty($departure_time_option)) {
			$departure_time_item = array(
				'text'  => esc_html__('Reisedauer', 'setsail-tours'),
		...
		if(!empty($return_time_option)) {
			$return_time_item = array(
				'text'  => esc_html__('Reisebeginn', 'setsail-tours'),
		...
		if(!empty($dress_code_option)) {
			$dress_code_item = array(
				'text'  => esc_html__('Reiseroute', 'setsail-tours'),
				'value' => $dress_code_option
			);

			$data[] = $dress_code_item;
		}
		$reisepreis_option = get_post_meta($tour_id, 'tour_price', true);

		if(!empty($reisepreis_option)) {
			$reisepreis_item = array(
				'text'  => esc_html__('Reisepreis', 'setsail-tours'),
				'value' => $reisepreis_option
			);
			$data[] = $reisepreis_item;
		}
		$hightlights_option = get_post_meta($tour_id, 'tour_discount_price', true);

		if(!empty($hightlights_option)) {
			$hightlights_item = array(
				'text'  => esc_html__('Highlights', 'setsail-tours'),
				'value' => $hightlights_option
			);

			$data[] = $hightlights_item;
		}
		$notiz_option = get_post_meta($tour_id, 'tour_custom_label', true);

		if(!empty($notiz_option)) {
			$notiz_item = array(
				'text'  => esc_html__('Notiz', 'setsail-tours'),
				'value' => $notiz_option
			);
